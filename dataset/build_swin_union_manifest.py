...
=====================================================
Flattens every prepared corpus that carries a manifest into one
...
(``corpus``, ``highres_corpus``, ``modern_corpus``, ``modern_v2``,
``legacy_archive``, ``gpt_nano``). Each corpus is internally split, but the
...
Only two corpora record sha256 natively (``corpus``, ``highres_corpus``,
``modern_corpus``, ``modern_v2``). ``legacy_archive`` and ``gpt_nano`` store
plain images, so hashes are computed once here and cached to a sidecar JSON,
making rebuilds cheap.

Dedup policy (priority train > val > test > holdout)
-----------------------------------------------------
Rows are grouped by sha256. For a hash that appears in several splits, the
copy in the highest-priority split is kept and the others dropped — so the same
pixels can never be both fitted and scored, and never counted twice in the same
split. The number of dropped rows is reported per split so the cost of leak
protection is on the record.
...
``path`` is absolute (the union spans six corpus roots; relative paths would be
...
SPLIT_PRIORITY = {"train": 0, "val": 1, "test": 2, "holdout": 3}
...
def rows_corpus_style(corpus: Path, name: str) -> List[dict]:
...
def rows_modern_v2(corpus: Path) -> List[dict]:
...
def dedup(rows: List[dict]) -> tuple[List[dict], dict]:
    """Drop duplicate hashes; keep the copy in the highest-priority split."""
...
def main() -> int:
    ...
    extractors = [
        ("corpus", lambda c: rows_corpus_style(c, "corpus")),
        ("highres_corpus", lambda c: rows_corpus_style(c, "highres_corpus")),
        ("modern_corpus", lambda c: rows_corpus_style(c, "modern_corpus")),
        ("modern_v2", rows_modern_v2),
